    ...
    class List {
       ...
       public:
          ...
          explicit List(int num, const T& val = T{})
          {
                   ...
                   // initializes 
            	init();
        	iterator itr = begin(); // begins iterator
				
		// runs through loop adding val to each slot 
		for(int i = 0; i < num; ++i){
		...
		// loops trough list pusinghback elements
               while(start != end)
               {
                 push_back(*start);
		...
		init(); // initializes 
		for(const T& val : iList){ // loops trough list 

		push_back(val); // inserts val into each spot 
    		}
		...
		// sets list values to initializer list values
